...
**Flow Matching**[^1] é um framework de geração que treina um campo de velocidade $v_\theta$ para transformar uma distribuição simples (ruído Gaussiano $p_0$) em uma distribuição de dados complexa ($p_1$) ao longo de uma trajetória contínua no tempo.
...
<div style="display:flex;gap:1rem;justify-content:center;margin-top:1rem;flex-wrap:wrap;">
  <button id="flow-btn" onclick="toggleFlow()" style="padding:6px 24px;background:#f0883e;color:#0d1117;border:none;border-radius:6px;cursor:pointer;font-weight:bold;">&#9654; Animar</button>
  <button onclick="resetFlow()" style="padding:6px 20px;background:#21262d;color:#c9d1d9;border:1px solid #30363d;border-radius:6px;cursor:pointer;">&#8635; Resetar</button>
...
(function() {
  ...
  window.toggleFlow = function() {
    running = !running;
    document.getElementById('flow-btn').textContent = running ? '⏸ Pausar' : '▶ Animar';
  ...
  window.resetFlow = function() {
    ...
    document.getElementById('flow-btn').textContent = '▶ Animar';
  ...
  function animate() {
    ...
    else { running = false; document.getElementById('flow-btn').textContent = '▶ Animar'; }
  }
...
**FLUX.1**[^2] (Black Forest Labs) usa Flow Matching com **Diffusion Transformers (DiT)** — substituindo a U-Net por blocos Transformer puros:
...
--8<-- "docs/2026.2/classes/flow-matching/quiz.pt.md"
